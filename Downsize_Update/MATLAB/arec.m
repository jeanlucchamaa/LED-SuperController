classdef (CaseInsensitiveProperties=true, TruncatedProperties=true) ...
        arec < hgsetget
%arec Audio recorder object.
%   arec creates an 8000 Hz, 8-bit, 1 channel arec object.
%   A handle to the object is returned.
%
%   arec(Fs, NBITS, NCHANS) creates an arec object with 
...
%
%   arec(Fs, NBITS, NCHANS, ID) creates an arec object using 
%   audio device identifier ID for input.  If ID equals -1 the default input 
%   device will be used.
%   
% arec Methods:
%   get            - Query properties of arec object.
...
%   set            - Set properties of arec object.
%   stop           - Stop recording.
%
% arec Properties:
...
%   Type             - Name of the class: 'arec'. (Read-only)
...
%   
% arec Properties (Deprecated):
%   NOTE: arec ignores any specified values for these properties, 
...
%       
    ...
    properties(GetAccess='public', SetAccess='private', Transient)
        Type = 'arec'  % For Backward compatibility
    end
    
    % --------------------------------------------------------------------
    % Unused Legacy Properties
    % These Properties are unused the current arec
    ...
    methods(Access='public')
        function obj = arec(sampleRate,numBits,numChannels,deviceID)
            narginchk(0,4);

            obj.DeviceID = obj.DefaultDeviceID;

            if nargin == 1 || nargin == 2
                error(message('MATLAB:audiovideo:arec:incorrectnumberinputs'));
            end
    ...
    methods
        function set.BitsPerSample(obj, value)
            if value ~= 8 && value ~= 16 && value ~= 24,
                error(message('MATLAB:audiovideo:arec:bitsupport'));
            end
        ...
        function set.DeviceID(obj, value)
            if ~(isscalar(value) && ~isinf(value))
                error(message('MATLAB:audiovideo:arec:NonscalarDeviceID'));
            end
            ...
            if (isempty(inputs))
                error(message('MATLAB:audiovideo:arec:noAudioInputDevice'));
            end
            
            if ~(value==obj.DefaultDeviceID || ismember(value, [inputs.ID]))
                error(message('MATLAB:audiovideo:arec:InvalidDeviceID'));
            end
        ...
        function set.SampleRate(obj, value)
            if value <= obj.MinimumSampleRate || value > obj.MaximumSampleRate
                error(message('MATLAB:audiovideo:arec:invalidSampleRate', obj.MinimumSampleRate, obj.MaximumSampleRate));
            end

            if ~(isscalar(value) && ~isinf(value))
                error(message('MATLAB:audiovideo:arec:NonscalarSampleRate'));
            end
        ...
        function set.NumberOfChannels(obj, value)
           if ~isscalar(value) || ~(value > 0 && value <= obj.MaximumNumChannels),
               error(message('MATLAB:audiovideo:arec:numchannelsupport'));
           end
        ...
        function set.TimerPeriod(obj, value)
            ...
            if(value < obj.MinimumTimerPeriod) 
                 error(message('MATLAB:audiovideo:arec:invalidtimerperiod'));
            end
        ...
        function set.Tag(obj, value)
            if ~(ischar(value))
                error(message('MATLAB:audiovideo:arec:TagMustBeString'));
            end
            obj.Tag = value;
        end
    ...
    methods(Access='private', Static)
        function validateFcn(fcn)
            if ~internal.Callback.validate(fcn)
                error(message('MATLAB:audiovideo:arec:invalidfunctionhandle'));
            end
        end
  
        function exp = createDeviceException(exception)
            msg = strrep(exception.message, 'PortAudio', 'Device');
            exp = MException('MATLAB:audiovideo:arec:DeviceError', msg);
        end
    end
    ...
    methods(Access='public')
        function c = horzcat(varargin)
            %HORZCAT Horizontal concatenation of arec objects.
            
            if (nargin == 1)
                c = varargin{1};
            else
                error(message('MATLAB:audiovideo:arec:noconcatenation'));
            end
        end
        
        function c = vertcat(varargin)
            %VERTCAT Vertical concatenation of arec objects.
        ...
        function status = isrecording(obj)
            ...
            %
            %    See also arec, AUDIODEVINFO, arec/GET,
            %             arec/SET.
        ...
        function stop(obj)
            ...
            %
            %    See also arec, AUDIODEVINFO, arec/GET,
            %             arec/SET, arec/RECORD,
            %             arec/RECORDBLOCKING, arec/PAUSE,
            %             arec/RESUME
        ...
        function record(obj, numSeconds)
            ...
            %       p = play(r);   % listen to complete recording
            %
            %    See also arec, arec/PAUSE,
            %             arec/STOP, arec/RECORDBLOCKING.
            %             arec/PLAY, arec/RESUME.
            ...
            if nargin == 2
                if isempty(numSeconds) || ~isnumeric(numSeconds) || (numSeconds <= 0)
                       error(message('MATLAB:audiovideo:arec:recordTimeInvalid'));
                end
        ...
        function recordblocking(obj, numSeconds)
            ...
            % Error checking.
            if ~isa(obj, 'arec')
                error(message('MATLAB:audiovideo:arec:noarecObj'));
            end
        ...
        function data = getaudiodata(obj, dataType)
            %GETAUDIODATA Gets recorded audio data in arec object.
            %
            ...
            %
            %    See also arec, AUDIODEVINFO, arec/RECORD.
            ...
            if ~ischar(dataType)
                error(message('MATLAB:audiovideo:arec:unsupportedtype'));
            end
            ...
            if ~isempty(err)
                error(message('MATLAB:audiovideo:arec:ChannelReadError'));
            end
            ...
            if (isempty(obj.AudioData))
                error(message('MATLAB:audiovideo:arec:recorderempty'));
            end
        ...
        function ap = getplayer(obj)
            ...
            %    this arec object.
            %
            %    See also arec, AUDIOPLAYER.

            ap = audioplayer(obj);
        end

        function player = play(obj, varargin)
        %PLAY Plays recorded audio samples in arec object.
        %
